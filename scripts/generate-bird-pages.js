...
const DEFAULTS = {
  csv: "/Users/juangranda/Downloads/mbw_recent_bird_sightings_mindo - bird_quest_mvp.csv",
  templateEn: "/Users/juangranda/Documents/Codex/2026-07-07/i/outputs/cloudflare_deploy_fix_bird_quest_2026_07_08/bird-quest/index.html",
  templateEs: "/Users/juangranda/Documents/Codex/2026-07-07/i/outputs/cloudflare_deploy_fix_bird_quest_2026_07_08/es/reto-de-aves/index.html",
  ...
  outDir: "/Users/juangranda/Documents/Codex/2026-07-07/i/outputs/bird_quest_39_facts_refresh_2026_07_08"
};

const BIRD_DATA_URL = "/assets/data/bird-quest.json";
...
function displayPhrase(value, lang) {
  const labels = {
    both: ["half-day and full-day routes", "rutas de medio día y día completo"],
    full_day: ["full-day routes", "rutas de día completo"],
    half_day: ["half-day routes", "rutas de medio día"],
    custom_only: ["custom private routes", "rutas privadas personalizadas"],
    early_morning: ["early morning", "temprano en la mañana"],
    morning: ["morning", "la mañana"],
    dawn: ["dawn", "el amanecer"],
    easy: ["beginner-friendly", "amigable para principiantes"],
    moderate: ["moderate challenge", "reto moderado"],
    hard: ["guide-assisted challenge", "reto con guía"],
    specialist: ["a specialist target", "un objetivo especializado"]
  };
  const key = clean(value);
  if (labels[key]) return lang === "es" ? labels[key][1] : labels[key][0];
  return key.replace(/_/g, " ");
}

function fallbackFacts(row, lang) {
  const name = clean(lang === "es" ? row.common_name_es : row.common_name_en) || clean(row.common_name_en);
  const habitat = clean(row.habitat_primary);
  const route = clean(lang === "es" ? row.route_cluster_name_es : row.route_cluster_name_en) || clean(row.route_cluster_name_en);
  const timing = displayPhrase(row.best_time_of_day, lang);
  const routeFit = displayPhrase(row.tour_visibility, lang);
  const difficulty = displayPhrase(row.target_difficulty, lang);

  if (lang === "es") {
    return [
      `${name} ayuda a los visitantes a practicar observación cuidadosa en ${habitat || "el bosque nublado"} antes del tour.`,
      route ? `Suele encajar mejor en ${route}, especialmente durante ${timing || "la ventana recomendada"}.` : `Conviene buscarlo durante ${timing || "la ventana recomendada"} con apoyo del guía.`,
      `En el reto funciona como una especie ${difficulty || "de aprendizaje"} para comparar forma, color, canto y comportamiento.`
    ];
  }

  return [
    `${name} helps guests practice careful watching in ${habitat || "cloud forest habitat"} before the tour.`,
    route ? `It is most useful to study for ${route}, especially during the ${timing || "recommended"} window.` : `It is best searched for during the ${timing || "recommended"} window with guide support.`,
    `In the quest, this works as a ${difficulty || "learning"} species for comparing shape, color, call, and behavior.`
  ];
}

function rowToBird(row) {
  const factsEn = splitFacts(row, "en");
  const factsEs = splitFacts(row, "es");
  const kidFactEn = clean(row.kid_fact_en);
  const kidFactEs = clean(row.kid_fact_es);
  const idTipEn = clean(row.id_tip_en);
  const idTipEs = clean(row.id_tip_es);
  return {
    ...
    factsEn: factsEn.length ? factsEn : fallbackFacts(row, "en"),
    factsEs: factsEs.length ? factsEs : fallbackFacts(row, "es"),
    kidFactEn: kidFactEn || (factsEn[0] || fallbackFacts(row, "en")[0]),
    kidFactEs: kidFactEs || (factsEs[0] || fallbackFacts(row, "es")[0]),
    idTipEn: idTipEn || (factsEn[1] || fallbackFacts(row, "en")[1]),
    idTipEs: idTipEs || (factsEs[1] || fallbackFacts(row, "es")[1]),
...
function replaceBirdData(html, birds) {
  return html.replace(
    /<script id="birdQuestData" type="application\/json"(?:\s+data-src="[^"]*")?>[\s\S]*?<\/script>/,
    `<script id="birdQuestData" type="application/json" data-src="${BIRD_DATA_URL}">${safeScriptJson(birds)}</script>`
  );
}
...
function updateRuntimeVersion(html) {
  return html.replace(/\/assets\/js\/bird-quest-page\.js(?:\?v=[^"]*)?/g, "/assets/js/bird-quest-page.js?v=2");
}

function renderTemplate(templatePath, birds, lang) {
  let html = fs.readFileSync(templatePath, "utf8");
  html = moveHowToSectionAboveCards(html);
  html = updateRuntimeVersion(html);
...
- /assets/data/bird-quest.json
- /assets/js/bird-quest-page.js
...
- JSON data workflow: Pass. The page now fetches /assets/data/bird-quest.json first and uses embedded data as a fallback.
...
- Deploy/overwrite the existing live EN and ES index.html files once, plus /assets/js/bird-quest-page.js and /assets/data/bird-quest.json.
- For future image/audio/fact-only updates, upload only /assets/data/bird-quest.json after regenerating it from the CSV.
...
function main() {
  ...
  const jsonOut = path.join(CONFIG.outDir, "assets", "data", "bird-quest.json");
  ...
  writeOutput(jsonOut, `${JSON.stringify({ generatedAt: new Date().toISOString(), count: birds.length, birds }, null, 2)}\n`);
